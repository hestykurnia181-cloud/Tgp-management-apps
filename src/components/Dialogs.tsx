...
export const CreateBusinessDialog: React.FC<{
  ...
  const getRecommendedModules = (tmpl: BusinessTemplate): BusinessModule[] => {
    switch (tmpl) {
      ...
      case BusinessTemplate.SERVICE:
        return [
          BusinessModule.POS,
          BusinessModule.INVENTORY, // Keep inventory available so user can manage service supplies/stock!
  ...
  const handleTemplateChange = (tmpl: BusinessTemplate) => {
    setTemplate(tmpl);
    // Apply template recommendation while keeping user freedom
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-3 text-xs">
          ...
          {type === 'RAW_MATERIAL' && (
            <div className="p-2.5 bg-amber-50 border border-amber-200/80 rounded-xl text-[11px] text-amber-900">
              <strong>Gudang Bahan Baku:</strong> Item ini khusus untuk bahan mentah olahan produksi BOM. Stok tidak akan muncul di kasir POS dan disimpan terisolasi di Gudang Bahan Baku.
            </div>
          )}

          {type === 'PRODUCT' && (
            <div className="p-2.5 bg-blue-50 border border-blue-200/80 rounded-xl text-[11px] text-blue-900">
              <strong>Gudang Stok:</strong> Barang siap jual di kasir POS. Penjualan di POS akan memotong stok di Gudang Stok secara otomatis.
            </div>
          )}
          ...
          <div>
            ...
            <input
              ...
              placeholder={
                type === 'RAW_MATERIAL'
                  ? 'Contoh: Biji Kopi Robusta / Daging Ayam'
                  : type === 'SERVICE'
                  ? 'Contoh: Cuci Motor / Potong Rambut / Jasa Servis'
                  : 'Contoh: Kopi Susu Aren / Snack Keripik'
              }
              required
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <input
                ...
                placeholder="kg / gr / liter / ml / pcs / sesi"
                className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
          ...
          <div className="py-8 text-center text-slate-500">
            ...
            {onOpenCreateItemModal && (
              ...
              >
                ...
                <span>+ Tambah Bahan Jadi / Menu Olahan</span>
              </button>
            )}
          </div>
        ) : (
