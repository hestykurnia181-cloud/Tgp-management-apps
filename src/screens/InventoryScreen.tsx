...
export const InventoryScreen: React.FC = () => {
  ...
  const [addItemInitialType, setAddItemInitialType] = useState<
    'PRODUCT' | 'SERVICE' | 'RAW_MATERIAL' | 'MENU_DISH'
  >('PRODUCT');
  const [addItemInitialLocation, setAddItemInitialLocation] = useState('Gudang Stok');

  const [showProduceModal, setShowProduceModal] = useState(false);
  const [showRestockModal, setShowRestockModal] = useState(false);
  const [preselectedRawId, setPreselectedRawId] = useState<string | undefined>(undefined);
  ...
  const openAddItemModal = (
    type: 'PRODUCT' | 'SERVICE' | 'RAW_MATERIAL' | 'MENU_DISH' = 'PRODUCT',
    loc = 'Gudang Stok'
  ) => {
    setAddItemInitialType(type);
    setAddItemInitialLocation(loc);
    setShowAddItemModal(true);
  };

  // Filtered lists: All items not RAW_MATERIAL belong to Gudang Stok (PRODUCT, MENU_DISH, FINISHED_GOODS, SERVICE)
  const finishedGoods = activeItems.filter((i) => i.type !== 'RAW_MATERIAL');
  ...
  return (
    <div className="space-y-5 pb-16">
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {canProduce && (
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            <button
              onClick={() => openAddItemModal('PRODUCT', 'Gudang Stok')}
              data-testid="btn_add_stock_item_header"
            ...
            <button
              onClick={() => openAddItemModal('RAW_MATERIAL', 'Gudang Bahan Baku')}
              data-testid="btn_add_raw_header"
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold transition shadow-xs"
            >
              <Plus className="w-4 h-4" />
      ...
      <div className="bg-white rounded-3xl p-4 border border-slate-200/80 shadow-xs space-y-3">
        ...
        {(activeTab === 'FINISHED' || activeTab === 'RAW') && (
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-1">
            ...
            {canProduce && activeTab === 'FINISHED' && (
              <button
                onClick={() => openAddItemModal('PRODUCT', 'Gudang Stok')}
                data-testid="btn_add_finished_stock_tab"
                className="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition shadow-xs whitespace-nowrap"
              >
                <Plus className="w-4 h-4" />
                <span>+ Tambah Barang Stok</span>
              </button>
            ...
            {canProduce && activeTab === 'RAW' && (
              <div className="flex items-center gap-2">
                <button
                  onClick={() => openAddItemModal('RAW_MATERIAL', 'Gudang Bahan Baku')}
                  data-testid="btn_add_raw_tab"
                  className="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold transition shadow-xs whitespace-nowrap"
                >
                  <Plus className="w-4 h-4" />
                ...
                <button
                  onClick={() => {
                    setPreselectedRawId(undefined);
                    setShowRestockModal(true);
                  }}
                  data-testid="btn_restock_raw_tab"
                  className="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold transition shadow-xs whitespace-nowrap"
                >
                  <Plus className="w-4 h-4" />
                  <span>Restock</span>
                </button>
        ...
        <div className="space-y-3">
          {filteredFinished.length === 0 ? (
            <div className="p-8 text-center bg-white rounded-3xl border border-slate-200">
              ...
              <p className="text-sm font-bold text-slate-800">Belum ada barang di Gudang Stok</p>
              <p className="text-xs text-slate-500 mt-1 mb-4">
                Tambahkan produk dagang atau menu olahan siap jual ke persediaan gudang.
              </p>
              {canProduce && (
                <button
                  onClick={() => openAddItemModal('PRODUCT', 'Gudang Stok')}
                  data-testid="btn_add_item_empty_state"
                ...
                >
                  <Plus className="w-4 h-4" />
                  <span>+ Tambah Barang Stok Sekarang</span>
                </button>
              )}
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="text-right">
                      <span className="text-base font-black text-purple-600 block">
                        {item.type === 'SERVICE' ? 'Tersedia' : `${item.stockQuantity} ${item.unit}`}
                      </span>
        ...
        <div className="space-y-3">
          ...
          {filteredRaw.length === 0 ? (
            <div className="p-8 text-center bg-white rounded-3xl border border-slate-200">
              ...
              <p className="text-sm font-bold text-slate-800">Belum ada bahan mentah di Gudang Bahan Baku</p>
              <p className="text-xs text-slate-500 mt-1 mb-4">
                Daftarkan bahan baku mentah untuk kebutuhan formula resep produksi BOM.
              </p>
              {canProduce && (
                <button
                  onClick={() => openAddItemModal('RAW_MATERIAL', 'Gudang Bahan Baku')}
                  data-testid="btn_add_raw_empty_state"
                  className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold transition shadow-xs"
                >
                  <Plus className="w-4 h-4" />
                  <span>+ Tambah Bahan Baku Mentah Sekarang</span>
                </button>
              )}
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs">
                    ...
                    {canProduce && (
                      <button
                        onClick={() => {
                          setPreselectedRawId(raw.itemId);
                          setShowRestockModal(true);
                        }}
        ...
        <div className="space-y-3">
          {finishedGoods.length === 0 ? (
            <div className="p-8 text-center bg-white rounded-3xl border border-slate-200">
              ...
              <p className="text-sm font-bold text-slate-800">Belum ada item barang jadi / menu olahan</p>
              <p className="text-xs text-slate-500 mt-1 mb-4">
                Tambahkan produk atau menu olahan di Gudang Stok untuk mengatur komposisi resep BOM.
              </p>
              {canProduce && (
                <button
                  onClick={() => openAddItemModal('MENU_DISH', 'Gudang Stok')}
                  className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold transition shadow-xs"
                >
                  <Plus className="w-4 h-4" />
                  <span>+ Tambah Menu Olahan (BOM)</span>
                </button>
              )}
            </div>
          ) : (
      ...
      <AddItemDialog
        ...
        initialLocation={addItemInitialLocation}
      />
      <ProduceGoodsDialog
        isOpen={showProduceModal}
        onClose={() => setShowProduceModal(false)}
        businessId={activeBusiness?.businessId}
        onOpenCreateItemModal={() => openAddItemModal('MENU_DISH', 'Gudang Stok')}
        onOpenRecipeModal={(itemId) => {
          const it = activeItems.find((x) => x.itemId === itemId);
          if (it) setSelectedRecipeItem(it);
        }}
      />
      <RestockRawMaterialDialog
        ...
        businessId={activeBusiness?.businessId}
        preselectedRawId={preselectedRawId}
        onOpenCreateItemModal={() => openAddItemModal('RAW_MATERIAL', 'Gudang Bahan Baku')}
      />
      {selectedRecipeItem && (
        <ConfigureRecipeBomDialog
